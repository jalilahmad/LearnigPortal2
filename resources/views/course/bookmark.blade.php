...
<!-- Popular -->
            ...
            @foreach ($user->bookmark as $course)
                    ...
                    <div class="card">
                        ...
                        <div class="price_box d-flex flex-row align-items-center">
                            <div class="course_author_image">
                                ...
                                <img src="images/author.jpg" alt="https://unsplash.com/@mehdizadeh">
                                    @else
                                    <img src="{{$course->User->profile_image}}" alt="images/author.jpg">
                                    @endif
                            ...
                            <div class="course_price d-flex flex-column align-items-center justify-content-center"><span>{{$course->Course_Price}}</span></div>
                                ...
                                <form method = "post" action = "{{route('remove.bookmark', $course->id)}}">
                                @csrf
                                <button style="border:none;background-color: #fdb506;" type="submit">
                                <i class="fas fa-bookmark" style="color: black;font-size:24px"></i>
                                </button>
                                </form>
                                </div>
                                @endif
                        ...
                        <strong>Tag:</strong>
                             @foreach($course->tags as $tag)
                                 <a href="{{route('tag.course', $tag->id)}}" class="label label-info">{{ $tag->name }}</a>
                             @endforeach
                        </div>
